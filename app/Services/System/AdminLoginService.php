...
class AdminLoginService
{
    ...
    public function login(array $data): JsonResponse
    {
        ...
        $cookie = null;
        if ($this->cookie) {
            $cookie = Cookie::make(config('veladmin.admin_login_cookie'), $user['id'], 24 * 60 * 10);
        }
    ...
    public function loginSession($id): bool
    {
        ...
        $root_admin_id = config('veladmin.root_admin_id');
        if ($user['id'] == $root_admin_id) {
            $is_admin = 1;
        }
        //非超管时，获取角色
        if (! $is_admin) {
            $root_role_id = config('veladmin.root_role_id');
        ...
        session()->flush();
        session()->put(config('veladmin.admin_login_session'), $sessionData);
    ...
    {
        session()->flush();
        Cookie::queue(cookie()->forget(config('veladmin.admin_login_cookie')));
    }
